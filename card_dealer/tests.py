...
class DealerTests(unittest.TestCase):
    ...
    def testDealNumberOfPlayers(self):
        deal = self.dealer.deal_to_players(Deck(), [Player('a'), Player('b')])
        self.assertEqual(len(deal), 2)

    def testDealPlayerNames(self):
        deal = self.dealer.deal_to_players(Deck(), [Player('a'), Player('b')])
        self.assertTrue('a' in deal and 'b' in deal)

    def testDealNumberOnTable(self):
        deal = self.dealer.deal_on_table(Deck())
        self.assertEqual(len(deal), 5)
